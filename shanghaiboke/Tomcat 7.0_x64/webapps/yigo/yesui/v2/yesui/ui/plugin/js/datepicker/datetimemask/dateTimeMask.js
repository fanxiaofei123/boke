...
//		event.preventDefault();
	    ...
	    switch (nKeyCode) {
	        case KEY.DEL:
	            ...
	            break; 
	        default :
	            if(!((nKeyCode >=48 && nKeyCode<=57) || (nKeyCode >=96 && nKeyCode<=105)) && nKeyCode != 8){
	            	event.preventDefault();
	            	return true;
	            }
...
//	            	return true;
				...
				if(nKeyCode >=48 && nKeyCode<=57) {
					...
					if(nCursorPos == 4) {
						...
						if(keycode == 3) {
							preText = options.preText.substr(0, nCursorPos+1) + "0" + options.preText.substr(nCursorPos+2, nTextLen);
						}
						text = objTextBox.value + keycode + preText.substr(nCursorPos+1, nTextLen); 
						if(!$.DateTimeMask.DealWith(options.masktype,text,nCursorPos)) {
							event.preventDefault();
							return true;
						}
					} else {
						var s = options.preText.substr(nCursorPos+1,nTextLen);
						if(nCursorPos + 1 == 9 && keycode > 0) {
							s = 0;
						}
						text = objTextBox.value + keycode + s; 
						if(!$.DateTimeMask.DealWith(options.masktype,text,nCursorPos)) {
							event.preventDefault();
							return true;
						}
...
//			}
	...
	isValidDate : function(strDate) {
		var ls_regex = "^((((((0[48])|([13579][26])|([2468][048]))00)|([0-9][0-9]((0[48])|([13579][26])|([2468][048]))))-02-29)|(((000[1-9])|(00[1-9][0-9])|(0[1-9][0-9][0-9])|([1-9][0-9][0-9][0-9]))-((((0[13578])|(1[0|2]))-31)|(((0[1,3-9])|(1[0-2]))-(29|30))|(((0[1-9])|(1[0-2]))-((0[1-9])|(1[0-9])|(2[0-8]))))))$";
