...
//		this.data = options.data.addedNodes;
	...
	onRender: function (ct) {
	        ...
	        this._$fuzzyText =  $('<input id="'+this.id+'_textbtn" type="text" placeholder="'+YIUI.I18N.dict.keyWord+'" />').addClass('txt').appendTo($fuzzybar);
			...
			this.handler.getDVInfo(form, this.getMetaObj())
						.then(function(dataObj){
							...
							if(self.isChainDict()) {
						    	...
						    	var maxRows = self.pageSize;
						    	var pageIndicatorCount = self.pageIndicatorCount;
						    	var value = self.getQueryValue();
								YIUI.DictService.getQueryData(itemKey, 0, maxRows, pageIndicatorCount, value, null, self.dictFilter, null)
								.then(function(data) {
						        ...
						        self.expandNode(root.attr('id'), true);
							}
						});
	},
...
//			$tr.click();
    ...
    syncHandleWidths: function() {
		...
		// 	var resizer = resizers.eq(i);
		// 	var th = $("th", _this._$table).eq(resizer.index());
		// 	resizer.css({
		// 		left: th.outerWidth() + th.position().left,
		// 		height: th.outerHeight()
		// 	});
